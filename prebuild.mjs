...
const getDeployUrl = async (branch) => {
  ...
  return result.channels.find(
    (deploy) => deploy.name.split("/").pop() === branch
  )?.url;
};
...
const generateBranchDeployUrl = async (branch) => {
  const { stdout } = await promisify(exec)(
    `firebase hosting:channel:create ${branch} --json`
  );
  const { result } = JSON.parse(stdout);
...
const setEnv = async (url) => {
  ...
  const stringified = Object.entries(envs).reduce(
...
prepare(process.argv[2] || process.env.CI_COMMIT_REF_SLUG).catch((e) => {
  console.error(e);
